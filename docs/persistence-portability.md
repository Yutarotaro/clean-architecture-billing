...
!!! success "実際にバグを 2 つ見つけました"
    インメモリ実装は `deepcopy` でデータベース全体のスナップショットを取り、
    `commit` で丸ごと置き換える方式でした。ここに欠陥が 2 つありました。

    **1 つ目**は契約テストを書いた時点で見つかりました。バージョンの比較を作業コピーに
    対して行っていたため、**他のトランザクションが先に更新したことに永遠に気づけない**。
    楽観ロックのテストだけが memory で落ちました。

    **2 つ目**はその後のコードレビューで見つかりました。`commit` がデータベース全体を
    置き換えるので、**自分が触っていない集約への他トランザクションの更新が消える**。
    楽観ロックは触った集約のバージョンしか見ないため、この巻き戻しは誰にも検出されません。
    既存の契約テストが同一の集約しか競合させていなかったので、見逃していました。

    最終的に、Go 版と同じ「変更した集約だけを staging に溜めて commit でマージする」
    方式に書き直しました。テスト用の実装であっても本物と同じ意味論を持たせなければ、
    インメモリで通ったテストの意味が消えます。

!!! warning "「触っていない集約」の扱いも実装ごとに違った"
    上の回帰テストを書いたことで、5 つ目の差が見つかりました。
    WAL モードの SQLite（Go 版）は「読んだ時点のスナップショットが古い」ことを検出し、
    **触っていない集約が動いただけでも書き込みを拒みます**。
    memory・DynamoDB・Python の SQLite（pysqlite は SELECT で BEGIN しない）は拒みません。

    安全側の失敗なので実害はありませんが、契約として要求できるのは
    「更新が失われないこと」までで、「後から書く側が必ず成功すること」は保証できません。
