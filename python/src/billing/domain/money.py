...
class Money:
    ...
    def scale(self, ratio: Fraction) -> Money:
        ...
        丸めは floor で統一する。日割りでは事業者側ではなく利用者側に有利な方向に
        倒す、という方針をここで一箇所に閉じ込めている。方針を変えたくなったら
        変えるのはこのメソッドだけで済む。

        0 方向への切り捨てではなく floor なのは、負の金額（プラン変更の明細では
        credit を負で持つ）でも Go 側と同じ結果になるようにするため。
        同じ入力に対して言語ごとに違う金額が出るのは、それだけで不具合である。
        """
        if ratio < 0:
